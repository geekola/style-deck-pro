    ...
    <div className="min-h-screen bg-white max-w-lg mx-auto">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-gray-400">
          ...
          <Link href="/app/discover" className="text-sm text-black underline">Start swiping</Link>
        </div>
      ) : (
        <div className="pb-12">
          {/* Category counts */}
          <div className="grid grid-cols-4 gap-2 px-5 py-4">
            {CATEGORY_ORDER.map((cat) => (
              <div key={cat} className="bg-gray-50 rounded-xl p-3 text-center">
                ...
                <div className="text-xs text-gray-400 capitalize mt-0.5">{cat}</div>
              </div>
            ))}
          </div>
                  ...
                  {catItems.map((item) => (
                    <div key={item.id} className="rounded-xl border border-black/8 overflow-hidden">
                      <div className="relative h-36 bg-gray-100">
                        ...
                        <span className="absolute top-2 right-2 text-xs bg-white/90 rounded-full px-2 py-0.5 font-medium">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          {item.active && (
                            <Link
                              ...
                              className="flex-1 text-center text-xs bg-black text-white rounded-lg py-1.5 font-medium"
                            >
